...
drawdag_jj <- function( x , col_arrow="black" , col_segment="black" , col_labels="black" , cex=1 , lwd=1.5 , goodarrow=TRUE , xlim , ylim , shapes , col_shapes , radius=3.5 , add=FALSE , xkcd=FALSE , latent_mark="c" , arr.width=1, arr.length=1, arr.adj=1, arr.type="simple", shift_arrows=TRUE, ... ){ 
    require(dagitty)
    ...
    for( i in seq_len(nrow(ex)) ){
        ...
        if (!shift_arrows) {
          ...
          ax1[i] <- x1 + x1*0.05; ax2[i] <- x1 + (x2 - x1)*0.95
        }
        if (!shift_arrows) {
          ay1[i] <- y1; ay2[i] <- y2
        } else {
          ay1[i] <- y1; ay2[i] <- y1 + (y2 - y1)*0.925
        }
    ...
    if ( goodarrow==TRUE ) {
        #require(shape)
      cut_fraction <- 0.95 
      # x_short <- x0 + (x1 - x0) * cut_fraction
      # y_short <- y0 + (y1 - y0) * cut_fraction
        shape::Arrows( ax1[directed], -ay1[directed], 
            ax2[directed], -ay2[directed], arr.length=arr.length , arr.width=arr.width, col=col_arrow , lwd=lwd , arr.adj=arr.adj , arr.type=arr.type )
    } else
        arrows( ax1[directed], -ay1[directed], 
            ax2[directed], -ay2[directed], length=arr.length, col=col_arrow , lwd=lwd )
...
dag_arc <- function(x1, y1, x2, y2, xm, ym, col = "gray", length = 0.1, code = 1, lwd = 1.5 , goodarrow=TRUE, arr.width=0.5, arr.length = 0.5, arr.adj=1.0) {
    x <- c(x1, xm, x2)
    y <- c(y1, ym, y2)
    ...
    if (code >= 3) {
        if ( goodarrow==TRUE ) {
            #require(shape)
            shape::Arrows( res$x[4], res$y[4], res$x[1], res$y[1], arr.length=arr.length , arr.width=arr.width, col=col , lwd=lwd , arr.adj=arr.adj , arr.type="curved" )
        } else
    ...
    if (code >= 2) {
        if ( goodarrow==TRUE ) {
            #require(shape)
            shape::Arrows( res$x[nr-3], res$y[nr-3], res$x[nr], res$y[nr], arr.length=arr.length , arr.width=arr.width, col=col , lwd=lwd , arr.adj=arr.adj , arr.type="triangle" )
        } else
...
drawopenpaths_jj <- function( x , Z=list() , col_arrow="red" , arr.width=1, arr.length=1, arr.adj=0, arr.type="simple", lwd=5, cex=5, ... ) {
    x <- as.dagitty( x )
    # get all paths
    ...
    for ( i in 1:length(path_list) ) {
        if ( path_list$open[i]==TRUE ) {
            ...
            drawdag_jj( path_dag , col_arrow=col_arrow , arr.width=arr.width, arr.length=arr.length, arr.adj=arr.adj, arr.type=arr.type, lwd=lwd, cex=cex, add=TRUE , ... )
        }
    }#i
...
drawdag_jj( plant_dag , cex=1.2 , col_labels=c("red","black","red","red") , col_arrow=c("red","black","red") , goodarrow=TRUE )
...
coordinates( exdag ) <- list( x=c(z=0,x=1,y=2,U=1.5) , y=c(z=0,x=0,y=0,U=-1) )
drawdag_jj( exdag , xkcd=TRUE , lwd=1.5 )
